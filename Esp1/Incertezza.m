...
%Il numero massimo di dati è 6, possono essere inseriti anche meno dati.
...
function incertezza(y1,dy1,y2,dy2,y3,dy3,y4,dy4,y5,dy5,y6,dy6,varargin)
%vengono passati i vettori di dati con relativa incertezza
syms f df x1 x2 x3 x4 x5 x6 dx1 dx2 dx3 dx4 dx5 dx6


f=log(x1); % la formula da testare



if ~exist('dy2', 'var') % controlla se esistono più variabili
    dy2 = 0*ones(size(y1));
end
...
if ~exist('dy3', 'var') % controlla se esistono più variabili
    dy3 = 0*ones(size(y1));
end
...
if ~exist('dy4', 'var') % controlla se esistono più variabili
    dy4 = 0*ones(size(y1));
end
...
if ~exist('dy5', 'var') % controlla se esistono più variabili
    dy5 = 0*ones(size(y1));
end
...
if ~exist('dy6', 'var') % controlla se esistono più variabili
    dy6 = 0*ones(size(y1));
end
...
dx1=diff(f,x1); % calcola le derivare
dx2=diff(f,x2);
dx3=diff(f,x3);
dx4=diff(f,x4);
dx5=diff(f,x5);
